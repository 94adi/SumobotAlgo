...
#ifndef SOURCES_HW_DRIVERS_SENSORS_TIME_OF_FLIGHT_TOF_H_
#define SOURCES_HW_DRIVERS_SENSORS_TIME_OF_FLIGHT_TOF_H_
...
#define      ALGO_PHASECAL_CONFIG_TIMEOUT                 0x30
    ...
    uint32 io_timeout;
    bool did_timeout;
    uint32 timeout_start_ms;
    uint8 stop_variable; // read by init and used when starting measurement; is StopVariable field of VL53L0X_DevData_t structure in API
    uint32 measurement_timing_budget_us;

    typedef enum  { VcselPeriodPreRange, VcselPeriodFinalRange }vcselPeriodType;

    uint8 last_status; // status of last I2C transmission
	 void setAddress(uint8 new_addr);//not using it
     inline uint8 getAddress(void) { return address; }
    ...
    inline uint16 getTimeout(void) { return io_timeout; }
    ...
    typedef struct
    {
     ...
     uint16 decodeTimeout(uint16 value);
...
#endif /* SOURCES_HW_DRIVERS_SENSORS_TIME_OF_FLIGHT_TOF_H_ */
