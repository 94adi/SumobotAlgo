...
void Io_Tpm_Initialization(const Io_Tpm_CnfStruct *Io_Tpm_Cnf)
{
	...
	for(i = 0; i < Io_Tpm_Cnf_Ptr->noOfTpms; i++)
	{
		...
		/* Set the counter starting value to 0*/
...
TPM_Type* Io_Tpm_getTpm(uint8 tpmDevice)
{
	...
	return temp; //contains the initialized register for the selected TPM channel
}
...
void Io_Tpm_Tpm1Isr(void)
{
	...
	/*	for(i=0;i<2;i++)
		...
		us_counter-=4;
		/*us_counter++;
				if(us_counter == 1000000) //5 seconds passed
				{
					us_counter = 0;
...
void delay_us(uint32 us)
{
	...
	mod = us_counter %4;
	us_counter -= mod;
	while(us_counter >0) {} //stay in the loop until us_counter reaches 0 (4 us at a time)
}
