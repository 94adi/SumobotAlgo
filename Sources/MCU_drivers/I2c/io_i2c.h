...
{
	while((i2c->S & I2C_S_IICIF_MASK)==0) {/*wait*/};
...
static inline uint16 i2c_get_ack(I2C_Type * i2c)
{
    if((i2c->S & I2C_S_RXAK_MASK) == 0)
        return TRUE; //acknowledge signal was received after the completion of one byte of data transmission on the bus
    else
        return FALSE; //no acknowledge signal detected
}
...
extern uint8 Io_I2c_RegWrite(I2C_Type * i2c,uint8 SlaveAddress, uint8 RegisterAddress, uint8 data);
extern uint8 Io_I2c_RegWrite2(I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress, uint8 * data);
extern uint8 Io_I2c_RegRead (I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress);
extern uint16 Io_I2c_RegRead16bit(I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress);
extern void Io_I2c_RegReadN (I2C_Type * i2c, uint8 SlaveAddress,uint8 RegisterAddress, uint8 N, uint8 *array);
extern void Io_I2c_WriteRegN(I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress, uint8 N, uint8 *array);
extern void Io_I2c_WriteReg16Bit(I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress, uint16_t value);
extern void Io_I2c_WriteReg32Bit(I2C_Type * i2c, uint8 SlaveAddress, uint8 RegisterAddress, uint32_t value);
